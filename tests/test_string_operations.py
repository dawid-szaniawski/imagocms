...
class TestChangeName:
    ...
    @pytest.mark.parametrize(
        ("filename", "expected_extension"), zip(filenames, expected_extensions)
    )
    def test_extension_should_be_correct(self, filename, expected_extension):
        assert filename.count(".") == 1
        assert (
            filename[-5:] == expected_extension or filename[-4:] == expected_extension
        )
    ...
    @pytest.mark.parametrize("filename", filenames)
    def test_filename_should_be_in_right_length(self, filename):
        assert len(filename) == 40 or len(filename) == 41
...
class TestCheckCorrectnessOfTheData:
    ...
    to_long_data = (
        (
            ...
            (random_string_generator() + "@wp.com"),
        ),
        (
            random_string_generator(),
            random_string_generator(257),
            (random_string_generator() + "@onet.pl"),
        ),
        (
            random_string_generator(),
            random_string_generator(),
            (random_string_generator(311) + "@gmail.com"),
        ),
        (
            random_string_generator(16),
            random_string_generator(257),
        ...
        (
            random_string_generator(312),
            random_string_generator(624),
            (random_string_generator(1248) + "@gmail.com"),
        ),
    )
    correct_data = (
        (
        ...
        (
            random_string_generator(),
            random_string_generator(),
            random_string_generator() + "@o2.pl",
        ),
        (
            random_string_generator(),
            random_string_generator(),
            random_string_generator() + "@gmail.com",
        ),
        (
            random_string_generator(),
            random_string_generator(),
    ...
    data_with_forbidden_char = (
        (
            random_string_generator(chars=forbidden),
            random_string_generator(),
            random_string_generator() + "@wp.pl",
        ),
        (
...
class TestPrepareSrcAndAlt:
    ...
    expected_data_values = (
        "Mieszkania",
        "Deszcz",
        "Trzeba zdenazyfikowac Watykan",
        "Bo nie zjadly kanapek",
        "piesek w pracy",
        "Wohoo",
    )
    ...
    def test_had_all_values(self, load_test_data, img_alt):
